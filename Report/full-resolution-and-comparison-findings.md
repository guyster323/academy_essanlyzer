...
~~사람이 재발행 여부를 판단할 것. 이 세션은 `Report/case_b_report.html`을 덮어쓰지 않았다.~~

**갱신 (같은 날, 사람 판단 완료): 재발행하기로 결정되어 반영했다.**
`Report/case_b_report.html`은 이제 위 실행에서 나온 **원본 해상도 독립 보고서**
(`tmp/fullres/case_b_report_fullres.html`와 동일 내용, 195,743자)이다. AI 호출을 다시
하지 않았다 — 같은 실행의 산출물을 그대로 발행했으므로 보고서 본문과 이 문서의 수치가
같은 런에서 나온 것임이 보장된다.

새 헤드라인:

> 2018-04-28~2022-01-10 **전 기간** 로그에서 U_Cell_8이 파생 전압편차 이상의 약
> 99%(744,400/751,686건)를 차지하고 편차 부호가 충·방전 전류 방향과 일치하며
> 2021-03~2021-12에 빈도·크기가 단조 증가해 'Cell 8 주 전류경로의 유효 직렬저항 증가'
> 후보로 판단되나, **휴지 상태 비영 편차(−0.038~−0.050 V)**와 이벤트 저항이 **Cell 1**을
> 지목하는 상충은 이 로그로 미해소다.

이전 공개본(stride80)과 달라진 점:
- 근거 구간이 "알람 근거 구간 커버리지 87.8%"에서 **전 기간**으로
- 파생 이상 9,366건 → **751,686건**
- 상충 상대 Cell 5 → **Cell 1**
- 인라인 그림 4개 → **3개** (B-F4가 원본 해상도에서 `unavailable`이 되었기 때문이며,
  누락이 아니라 정직한 반영이다)

**재발행해도 해소되지 않는 것**: 위 「캡이 결론에 영향을 주는가」에서 적었듯 B-F1의
Cell 1 지목은 여전히 qualifying 이벤트의 약 0.1%(603/4,000) 위에서만 성립한다. 새
보고서도 이 상충을 미해소로 명시한다. **저항 이벤트 캡 구조 재설계가 다음 후속
과제**이며, 재발행이 그것을 대신하지 않는다.
