...
test('draft-report prompt states evidence time range as fact and does not instruct a conclusion', () => {
  const profile = {
    sourceFile: 'data_sys_6.csv', formatId: 'lfp-cell-array',
    formatLabel: 'LFP cell-array 필드 데이터', entityColumn: null,
    rowCount: 20, derivedAlarmCount: 2,
  ...
  assert.match(report, /시간 커버리지 — 사실 기록/);
  assert.match(report, /데이터 2018-04-28 ~ 2021-09-15/);
  assert.match(report, /알람 근거 2018-10-10 ~ 2018-12-01/);
  assert.match(report, /커버리지 4%/);
  assert.match(report, /headline과 rootCause에 그 근거 구간을 명시/);
  assert.match(report, /unknownBox에 장기 거동이 이 근거로는 미확인/);
  assert.match(report, /결론을 지시하지 않는다/);
  assert.doesNotMatch(report, /500일/);
  assert.doesNotMatch(report, /1095/);
  assert.doesNotMatch(report, /3년/);
});

test('draft-report prompt omits the horizon block when no time ranges are present', () => {
  const report = buildDraftReportPrompt({
    issueStructured: {}, anomalyWindows: [], confirmedHyp: { claimLimit: 'limit' },
    finalSeverity: '중', finalSeverityReason: 'reason',
    sourceProfiles: [base]
  });
  assert.doesNotMatch(report, /시간 커버리지 — 사실 기록/);
});
